...
- 当前线上环境：Vercel + Prisma Postgres + Gemini API + 阿里云 ASR
...
- Prisma ORM
- PostgreSQL
- Gemini API（默认服务端 LLM）
- MiniMax / OpenAI-compatible（可选接入）
- 阿里云实时 ASR
- MCP TypeScript SDK
- `docx` 文档导出
...
- 默认建议只配置 Gemini API
- 当前线上示例使用的是 Google AI Studio 的 `Gemini 3 Flash`
- 前端运行时可临时切到 MiniMax 或 OpenAI-compatible，本地保存在浏览器中
