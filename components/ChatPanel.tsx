...
export default function ChatPanel() {
  ...
  const filteredTemplates = templateFilter
    ? filterTemplates(templateFilter)
    : templates;
  ...
  const selectTemplate = useCallback(
    (template: Template) => {
      setShowTemplates(false);
      setInput('');
      handleSend(template.name, template.prompt);
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [segments, userNotes, enhancedNotes, speakers]
  );

  const handleSend = async (displayText?: string, templatePrompt?: string) => {
    ...
    <div className="flex h-full flex-col">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {chatMessages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-zinc-400">
            ...
            <p className="mt-1 text-xs text-center max-w-[220px]">
              {hasContent
                ? '对会议内容提问，或输入 / 调用专业模版'
                : '开始录音后，可以在这里提问'}
            </p>
            {hasContent && (
              <div className="mt-4 flex flex-wrap gap-1.5 justify-center">
                {['会议的核心决策是什么？', '提取所有行动项', '总结关键讨论点'].map(
                  (q) => (
                    <button
                      key={q}
                      onClick={() => {
      ...
      {showTemplates && (
        <div className="border-t border-zinc-100 bg-white">
          <div className="px-3 py-1.5 text-xs font-medium text-zinc-400 flex items-center gap-1">
            <ChevronUp size={12} />
            选择模版
          </div>
      ...
      <div className="border-t border-zinc-100 p-3">
        <div className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 focus-within:border-zinc-400 transition-colors">
          <input
            ref={inputRef}
            value={input}
